    ...
    def generate_response(self, brain_result):
        ...
        if BRAIN_NATIVE_AVAILABLE and hasattr(self, "last_brain_response"):
            ...
            if hasattr(self, "last_brain_data"):
                ...
                neural_info = (
                    f" | Neural Intensity: {brain_data['neural_pattern']['intensity']:.3f}"
                )
...
async def process_brain_pattern(request: BrainProcessRequest):
    ...
    try:
        ...
        pattern = {
            int(k): {int(nk): nv for nk, nv in v.items()} for k, v in request.pattern.items()
        }
    ...
    try:
        ...
        return APIResponse(
            ...
            data={
                ...
                "restoration_success": {
                    "vocabulary_match": brain_state["vocabulary_size"]
                    == learning_stats.get("vocabulary_size", 0),
...
if __name__ == "__main__":
    uvicorn.run("simple_api:app", host="0.0.0.0", port=8000, reload=True, log_level="info")
