...
from api.models.responses import BrainProcessResponse, APIResponse
import time
import asyncio
...
async def process_neural_pattern(
    request: NeuralPatternRequest,
    ...
    try:
        ...
        return BrainProcessResponse(
            ...
            performance_metrics={
                ...
                "neurons_per_second": sum(len(neurons) for neurons in brain_pattern.values()) / (processing_time / 1000) if processing_time > 0 else 0
            }
        )
...
async def convert_text_to_pattern(
    request: TextToPatternRequest,
    brain=Depends(get_brain_network)
):
    """
...
async def simulate_brain_activity(
    ...
    brain=Depends(get_advanced_brain)
):
    """
    ...
    try:
        ...
        pattern = {int(k): {int(nk): nv for nk, nv in v.items()} 
                  for k, v in request.input_pattern.items()}
        ...
        return APIResponse(
            ...
            data={
                ...
                "intermediate_states": simulation_results if request.return_intermediate_states else [],
                "metrics": {
                    "total_time_ms": processing_time,
                    "steps_per_second": request.simulation_steps / (processing_time / 1000) if processing_time > 0 else 0
                } if request.track_metrics else {}
            }
        )
