...
from .simplified_brain_network import SimpleBrainNetwork, NetworkConfig, SimpleSpikingNeuron
...
class CognitiveBrainNetwork(SimpleBrainNetwork):
    ...
    def __init__(self, num_neurons: int, connectivity_prob: float = 0.1, 
    ...
    def _find_memory_associations(self, new_memory_id: str):
        ...
        for existing_id, existing_memory in self.episodic_memories.items():
            ...
            if association_strength > self.config.association_strength_threshold:
                ...
                print(f"🔗 Association created: {new_memory_id} ↔ {existing_id} (strength: {association_strength:.3f})")
    
    def _calculate_pattern_similarity(self, pattern1: Dict[int, bool], pattern2: Dict[int, bool]) -> float:
    ...
    def generate_inferences(self, query_pattern: Dict[int, bool], 
                          query_context: Dict[str, any] = None) -> List[Dict]:
        ...
        for i, inf in enumerate(inferences[:3]):  # Show top 3
            print(f"   {i+1}. Confidence: {inf['confidence']:.3f}, Chain length: {len(inf['inference_chain'])}")
        
        return inferences
    
    def _find_relevant_memories(self, pattern: Dict[int, bool], 
        ...
        for memory_id, memory in self.episodic_memories.items():
            ...
            # Overall relevance - boost context similarity for better matching
            relevance = (0.3 * pattern_sim + 0.5 * context_sim + 0.2 * memory_strength)
            
            if relevance > 0.05:  # Lower minimum relevance threshold
                relevance_scores.append((memory_id, relevance))
                print(f"     Memory {memory_id}: pattern_sim={pattern_sim:.3f}, context_sim={context_sim:.3f}, relevance={relevance:.3f}")
        
        # Sort by relevance
        relevance_scores.sort(key=lambda x: x[1], reverse=True)
        return relevance_scores
    
    def _explore_memory_associations(self, memory_id: str, explored: Set[str], 
                                   depth: int) -> List[str]:
    ...
    def _manage_memory_capacity(self):
        ...
        for memory_id, memory in self.episodic_memories.items():
            ...
            score = memory.consolidation_level + 0.1 * access_count + 0.05 * len(memory.associations)
    ...
    def step_with_cognition(self, external_input: Dict[int, bool] = None, 
        ...
        if context:  # Store memory whenever context is provided
            ...
            print(f"   📚 Stored memory: {context.get('concept', context.get('activity', 'unknown'))} (spikes: {spike_count})")
        else:
            memory_id = None
    ...
    def get_cognitive_state(self) -> Dict:
        ...
        return {
            'total_memories': len(self.episodic_memories),
            'avg_consolidation_level': np.mean(consolidation_levels) if consolidation_levels else 0,
            'avg_associations_per_memory': np.mean(association_counts) if association_counts else 0,
            'total_associations': sum(len(assocs) for assocs in self.memory_associations.values()) // 2,
            'consolidation_queue_size': len(self.consolidation_queue),
            'memory_capacity_usage': len(self.episodic_memories) / self.config.max_episodic_memories
        }
