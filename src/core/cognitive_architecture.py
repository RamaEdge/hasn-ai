...
class CognitiveArchitecture:
    ...
    def process_input(
        self, input_data: Union[str, List[float]], context: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        if arbitration_decision == "recall":
            ...
            if consolidation_result.get("consolidated"):
                ...
                print(
                    f"   Warning: Consolidation failed: {consolidation_result.get('reason', 'unknown')}"
                )
    ...
    def _check_consolidation_opportunities(self, trace_id: str) -> Dict[str, Any]:
        ...
        should_consolidate = self.executive_layer.should_consolidate(trace_id, trace.access_count)
        ...
        if should_consolidate:
            ...
            if len(concept_traces) >= self.config.min_concept_traces_for_consolidation:
                ...
                semantic_id = self.semantic_memory.consolidate_traces(all_traces, concept)
    ...
    def get_memory_statistics(self) -> Dict[str, Any]:
        ...
        avg_access_count = (
            np.mean([trace.access_count for trace in episodic_traces]) if episodic_traces else 0
        )

        # Semantic memory stats
        semantic_memories = list(self.semantic_memory.semantic_memories.values())
        avg_consolidated_count = (
            np.mean([mem.consolidated_count for mem in semantic_memories])
        ...
        return {
            ...
            "episodic_memory": {
                ...
                "oldest_trace": (
                    min([trace.timestamp for trace in episodic_traces]) if episodic_traces else None
                ),
                "newest_trace": (
                    max([trace.timestamp for trace in episodic_traces]) if episodic_traces else None
                ),
            },
            "semantic_memory": {
