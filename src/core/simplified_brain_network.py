...
class SimpleSpikingNeuron:
    ...
    def apply_learning(self, input_spikes: Dict[int, bool], learning_rate: float = 0.01, max_weight: float = 2.0):
...
class SimpleBrainNetwork:
    ...
    def __init__(self, num_neurons: int, connectivity_prob: float = 0.1, config: NetworkConfig = None):
    ...
    def _init_attention_system(self):
        ...
        if self.config.attention_init == "equal":
            self.attention_weights = np.ones(self.config.num_attention_modules) / self.config.num_attention_modules
    ...
    def step(self, external_input: Dict[int, bool] = None) -> Dict[int, bool]:
        ...
        if np.random.random() < self.config.learning_probability:
            for neuron in self.neurons:
                neuron.apply_learning(current_spikes, 
...
def demo_configurable_network():
    ...
    network = SimpleBrainNetwork(
        num_neurons=50, 
        connectivity_prob=0.1,
        config=config
    )
    
    print(f"Created fast-learning network:")
