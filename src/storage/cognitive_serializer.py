...
from ..core.cognitive_models import EpisodeTrace, SemanticMemory, WorkingMemoryItem
from ..core.snapshot_schema import BackendState, CognitiveLayerState, SnapshotMetadata, SnapshotV1
...
class CognitiveArchitectureSerializer:
    ...
    def save(
        ...
        for trace_id, trace in architecture.episodic_memory.traces.items():
            episodic_traces[trace_id] = {
                ...
                "last_consolidation": (
                    trace.last_consolidation.isoformat() if trace.last_consolidation else None
                ),
            }
    ...
    def load(
        self, snapshot_id: str, architecture: Optional[CognitiveArchitecture] = None
        ...
        if architecture is None:
            ...
            architecture = CognitiveArchitecture(
                config=config, backend_name=snapshot.metadata.backend_name
            )
        ...
        for trace_id, trace_data in episodic_data.get("traces", {}).items():
            trace = EpisodeTrace(
                ...
                last_consolidation=(
                    datetime.fromisoformat(trace_data["last_consolidation"])
                    if trace_data.get("last_consolidation")
                    else None
                ),
            )
        ...
        if "association_matrix" in associative_data:
            architecture.associative_layer.association_matrix = np.array(
                associative_data["association_matrix"]
            )

        # 3. Restore system state
        architecture.current_state.total_activations = snapshot.system_state["current_state"][
            "total_activations"
        ]
        architecture.current_state.consolidation_events = snapshot.system_state["current_state"][
            "consolidation_events"
        ]
        architecture.current_state.recall_events = snapshot.system_state["current_state"][
            "recall_events"
        ]
    ...
    def list_snapshots(self) -> List[Dict[str, Any]]:
        ...
        for json_file in self.storage_path.glob("*.json"):
            try:
                ...
                print(f"Warning: Skipping invalid snapshot {json_file.name}: {e}")
                continue
