...
class InteractiveBrainTrainer:
    ...
    def __init__(self, module_sizes: List[int] = [50, 40, 30, 20]):
        ...
        self.network = SimpleBrainNetwork(num_neurons=total_neurons, connectivity_prob=0.1)
        self.training_data = []
    ...
    def train_on_pattern(
        ...
        for iteration in range(num_iterations):
            result_spikes = self.network.step(input_pattern)
            result = self._normalize_step_output(result_spikes)
    ...
    def train_concept_association(
        ...
        for example in examples:
            ...
            self.concept_associations[concept].append(
                {"example": example, "pattern": pattern, "training_results": training_results}
            )
    ...
    def recognize_pattern(self, input_pattern: Dict[int, bool]) -> Tuple[str, float]:
        """
