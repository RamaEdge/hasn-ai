...
class TrainingMonitor:
    ...
    def analyze_learning_progress(self, metrics_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        concept_effectiveness = {
            ...
            for concept, scores in all_effectiveness.items()
        }
        
        analysis['most_effective_concepts'] = sorted(
            concept_effectiveness.items(),
            key=lambda x: x[1],
            reverse=True
        )[:10]
    ...
    def generate_training_report(self) -> str:
        ...
        if len(analysis['quality_trends']) > 1:
            ...
            if len(recent_quality) >= 2:
                quality_change = recent_quality[-1]['average_quality'] - recent_quality[0]['average_quality']
                trend = "📈 Improving" if quality_change > 0 else "📉 Declining" if quality_change < 0 else "➡️  Stable"
        ...
        if config:
            ...
            report.append(f"   Max Articles per Session: {config.get('max_articles_per_session', 'N/A')}")
    ...
    def create_visualization(self, save_path: str = None):
        ...
        try:
            ...
            if concept_data:
                ...
                ax1.bar(sessions, new_concepts, alpha=0.6, color='lightblue', label='New Concepts')
            ...
            if quality_data:
                ...
                ax2.fill_between(sessions, 
                                [q - v for q, v in zip(quality, variance)],
                                [q + v for q, v in zip(quality, variance)],
                                alpha=0.3, color='green', label='Quality Variance')
            ...
            if effective_concepts:
                ...
                concepts = [c[:15] + '...' if len(c) > 15 else c for c in concepts]  # Truncate long names
        ...
        try:
            while True:
                ...
                if metrics_data:
                    ...
                    print("   Start training with: python -m src.training.automated_internet_trainer")
