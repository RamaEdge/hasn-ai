...
class HyperparameterOptimizer:
    ...
    def suggest_parameters(self, trial: Trial) -> CognitiveConfig:
        ...
        background_noise_level = trial.suggest_float('background_noise_level', 0.001, 0.05, log=True)
        ...
        semantic_consolidation_threshold = trial.suggest_int('semantic_consolidation_threshold', 2, 10)
        max_confidence = trial.suggest_float('max_confidence', 0.7, 1.0)
        min_concept_traces_for_consolidation = trial.suggest_int('min_concept_traces_for_consolidation', 1, 5)
        
        # Sensory parameters
        background_spike_probability = trial.suggest_float('background_spike_probability', 0.001, 0.05, log=True)
        activation_pattern_threshold = trial.suggest_float('activation_pattern_threshold', 0.3, 0.7)
        
        # Executive parameters
        consolidation_priority_weight_access = trial.suggest_float('consolidation_priority_weight_access', 0.5, 0.9)
        consolidation_priority_weight_recency = trial.suggest_float('consolidation_priority_weight_recency', 0.1, 0.5)
    ...
    def evaluate_config(
        ...
        for item in training_data:
            ...
            if concept not in [m.concept for m in architecture.semantic_memory.semantic_memories.values()]:
                training_metrics["concepts_learned"] += 1
        
        training_time = time.time() - training_start
        
        # Count semantic memories
        training_metrics["semantic_memories_formed"] = len(architecture.semantic_memory.semantic_memories)
        ...
        if validation_data:
            ...
            validation_metrics = {
                "recall_accuracy": correct_recalls / total_validations if total_validations > 0 else 0.0,
                "validation_time": validation_time,
            }
        
        # Calculate composite performance score
        # Primary: Learning efficiency (concepts learned per second)
        learning_efficiency = training_metrics["concepts_learned"] / training_time if training_time > 0 else 0.0
        
        # Secondary: Consolidation quality (semantic memories formed)
        consolidation_quality = training_metrics["semantic_memories_formed"] / max(1, training_metrics["concepts_learned"])
        
        # Tertiary: Processing speed (average processing time)
        avg_processing_time = np.mean(training_metrics["processing_times"]) if training_metrics["processing_times"] else 1.0
