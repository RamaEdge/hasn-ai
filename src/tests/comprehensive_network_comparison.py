...
class NetworkComparator:
    ...
    def create_equivalent_networks(self, num_neurons: int = 100) -> Dict[str, Any]:
        ...
        try:
            start_time = time.time()
            networks["advanced"] = CognitiveBrainNetwork(
    ...
    def run_performance_test(
        self, network, network_name: str, duration: int = 1000
    ) -> Dict[str, Any]:
        """Run performance test on a network"""
        print(f"  Testing {network_name}...")

        results = {"network_name": network_name, "duration": duration, "success": False}
        ...
        try:
            ...
            if network_name == "simplified":
                sim_results = network.run_simulation(
                    duration=duration, input_pattern_func=create_input
                )
            ...
            results.update(
                {
                    ...
                    "spikes_per_second": (
                        spikes_recorded / execution_time if execution_time > 0 else 0
                    ),
                }
            )
    ...
    def comprehensive_comparison(self) -> Dict[str, Any]:
        ...
        return {
            "networks_created": {
                k: v is not None for k, v in networks.items() if not k.endswith("_time")
            },
    ...
    def generate_recommendations(self, performance: Dict, functionality: Dict) -> Dict[str, Any]:
        ...
        for network_name in ["simplified", "advanced", "optimized"]:
            ...
            if perf_data.get("success", False):
                ...
                memory_score = 1000 / max(
                    perf_data.get("memory_mb", 1), 1
        ...
        for network_name in performance_scores:
            perf_norm = (
                performance_scores[network_name]
                / max(performance_scores[network_name] for network_name in performance_scores)
                if max(performance_scores.values()) > 0
                else 0
            )
            func_norm = (
                functionality_scores[network_name] / max(functionality_scores.values())
                if max(functionality_scores.values()) > 0
                else 0
            )
            overall_scores[network_name] = (
                perf_norm * 0.6 + func_norm * 0.4
            )  # Weight performance higher
