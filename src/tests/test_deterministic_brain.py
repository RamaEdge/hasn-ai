...
from common.random import create_deterministic_context, seed_random_generators
from core.simplified_brain_network import NetworkConfig, SimpleBrainNetwork
...
class TestDeterministicBrainNetwork:
    ...
    def test_deterministic_initialization(self):
        ...
        assert (
            initial_weights1 == initial_weights2
        ), "Brain networks should initialize identically with same seed"
    ...
    def test_deterministic_spike_pattern(self):
        ...
        assert all(
            isinstance(k, int) for k in expected_output.keys()
        ), "Output keys should be integers"
        assert all(
            isinstance(v, bool) for v in expected_output.values()
        ), "Output values should be booleans"

        print(" Deterministic spike pattern test passed")
    ...
    def test_deterministic_learning(self):
        ...
        assert (
            outputs_after_training[0] == outputs_after_training[1]
        ), "Learning should be deterministic"

        print(" Deterministic learning test passed")
    ...
    def test_deterministic_raster_plot(self):
        ...
        print(" Deterministic raster plot test passed")
    ...
    def test_network_config_determinism(self):
        ...
        config1 = NetworkConfig(dt=1.0, learning_rate=0.01, num_attention_modules=3)

        config2 = NetworkConfig(dt=1.0, learning_rate=0.01, num_attention_modules=3)
        ...
        assert (
            output1 == output2
        ), "Networks with identical configs should produce identical outputs"

        print(" Network config determinism test passed")
    ...
    def test_known_spike_pattern_validation(self):
        ...
        assert all(
            output == outputs[0] for output in outputs
        ), "Known pattern should be deterministic"
        ...
        print(" Known spike pattern validation passed")
...
def test_full_deterministic_pipeline():
    ...
    logger.info(
        ...
        extra={"input_spikes": sum(input_pattern.values()), "output_spikes": sum(output.values())},
    )
