...
def run_cognitive_demo():
    ...
    test_scenarios = [
        ...
        (
            ...
            "Complex question - activates memory and executive systems",
        ),
    ...
    for i, (input_text, description) in enumerate(test_scenarios, 1):
        ...
        for wave_name, amplitude in result["brain_oscillations"].items():
            wave_state = (
                "Strong" if abs(amplitude) > 0.5 else "Moderate" if abs(amplitude) > 0.3 else "Weak"
            )
            print(
                f"  {wave_name.capitalize():>5} ({brain.brain_waves[wave_name]:>4.1f}Hz): {amplitude:>6.3f} ({wave_state})"
            )
