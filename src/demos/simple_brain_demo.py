...
class SimpleBrainNetwork:
    ...
    def step(self, external_inputs: Dict[int, Dict[int, bool]] = None) -> Dict:
        ...
        if total_activity > 0:
            for i, activity in module_activities.items():
                self.attention_weights[i] = 0.9 * self.attention_weights[i] + 0.1 * (activity / total_activity)
    ...
    def demonstrate_learning(self, num_steps: int = 1000) -> Dict:
        ...
        for step in range(num_steps):
            ...
            if step % 100 == 0:
                print(f"  Step {step}: Activity={result['total_activity']:.3f}, "
...
def main_demonstration():
    ...
    if analysis['plasticity_score'] > 0.5:
        print(f"   ✓ Network shows strong synaptic plasticity")
    else:
        print(f"   ⚠ Network shows limited plasticity - may need longer training")
        
    if analysis['attention_stability'] > 0.7:
        print(f"   ✓ Attention mechanism has stabilized effectively")
    else:
        print(f"   ⚠ Attention is still adapting - shows dynamic processing")
        
    if analysis['peak_memory_usage'] >= 5:
        print(f"   ✓ Working memory is being actively utilized")
    else:
        print(f"   ⚠ Low memory usage - input patterns may be too simple")
        
    if analysis['significant_changes'] > 20:
        print(f"   ✓ Network structure is adapting significantly")
    else:
        print(f"   ⚠ Limited structural changes - network may need more diverse inputs")
        
    print(f"\n🌟 BIOLOGICAL REALISM ACHIEVED:")
    realism_features = [
        f"🧠 Temporal spike-based processing (not continuous activation)",
        f"⚡ Activity-dependent synaptic strengthening",
        f"🎯 Selective attention mechanisms",
        f"💾 Limited capacity working memory",
        f"🔄 Homeostatic activity regulation",
        f"🌱 Structural plasticity and adaptation"
    ]
    ...
    applications = [
        f"🤖 Neuromorphic computing hardware",
        f"🎵 Temporal pattern recognition (speech, music)",
        f"🦾 Brain-computer interfaces",
        f"🧭 Adaptive robotics control",
        f"🧪 Cognitive modeling research",
        f"⚡ Ultra-low power AI systems"
    ]
    ...
    print(f"This novel architecture bridges neuroscience and AI,")
    print(f"offering a more biologically plausible approach to")
    print(f"artificial intelligence with temporal dynamics,")
    print(f"adaptive learning, and cognitive capabilities.")
    print(f"="*70)
