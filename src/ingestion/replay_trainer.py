...
class ReplayTrainer:
    ...
    def run_training_job(self, job_id: str) -> TrainingJob:
        ...
        try:
            ...
            job.metrics = {
                ...
                "average_spike_count": sum(m.get("spike_count", 0) for m in all_metrics) / max(len(all_metrics), 1)
            }
            
            logger.info(f"Job {job_id} completed: {items_processed} processed, {items_failed} failed")
    ...
    def consolidate(self, max_items: int = 100) -> Dict[str, Any]:
        ...
        items = self.quarantine_buffer.list_items(
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "jobs_completed": sum(1 for j in self.jobs.values() if j.status == TrainingJobStatus.COMPLETED),
            "jobs_failed": sum(1 for j in self.jobs.values() if j.status == TrainingJobStatus.FAILED)
        }
